...
updateLasso <- function(x, y, prvs, p = 10, maxiter = 100000, minerror = 10^(-5)){
  ...
  nobs <- nrow(x)
  ncov <- ncol(x)
  # Create an empty matrix to store all intermediate w values
  wmatrix <- matrix(, nrow = maxiter, ncol = ncov)
  # Initialization
  m <- 0
  ...
  if ((m > maxiter) & (delta > minerror)){
    ...
    beta <- p*w
    wmatrix <- remove_empty(wmatrix, which = "rows")
    output <- list("Niteration" = m, "coefficient" = beta, "allvalues" = wmatrix)
    return(output)
  }
}
...
newresult <- updateLasso(x = xstan, y = ystan, prvs = prvscoeff, p = 1)


# Plot the result out
library(ggplot2)
library(reshape2)
coeff_result <- as.data.frame(newresult$allvalues)
colnames(coeff_result) <- colnames(x)
coeff_result <- data.frame(iteration = seq(1:newresult$Niteration), coeff_result)
coeff_result <- melt(coeff_result, id.vars = "iteration")
newresult$coefficient - prvscoeff

horizontal <- prvscoeff[c(2, 3, 8, 9, 10)]
ggplot(coeff_result, aes(x = iteration, y = value, colour = variable)) + geom_line() +
  geom_hline(yintercept = horizontal, linetype = "dashed", color = "black")
