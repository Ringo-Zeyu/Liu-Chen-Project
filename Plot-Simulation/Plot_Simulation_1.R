...
for (s in 1:6){
  ...
  for(dt in 1:100){
    ...
    data.new <- data.all[2001:(2000+newsize), ]
    data.test <- data.all[(2000+newsize+1):(2000+newsize+200), ]
...
ggplot(data = graph.data.m, aes(x=variable, y=value)) + geom_boxplot(aes(fill=label)) +
  ...
  ggtitle("Distribution of MSE for Generating Mechanism 1 with different New Data size") +
