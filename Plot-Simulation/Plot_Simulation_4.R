...
for (s in 1:6){
  ...
  beta.n <- c(2.8, 1.3, 2.5, 1.2, 0, 0.3, 0)
  ...
  for(dt in 1:100){
    ...
    new.covariate <- matrix(, nrow = (newsize+200), ncol = 7)
    # Generate the error term 
...
ggplot(data = graph.data.m, aes(x=variable, y=value)) + geom_boxplot(aes(fill=label)) +
  ...
  ggtitle("Distribution of MSE for Generating Mechanism 4 with different New Data size") +
